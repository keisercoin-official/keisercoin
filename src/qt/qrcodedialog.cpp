...
{
    QString ret = QString("keisercoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MKC unit input in lnReqAmount, we generate the URI with MKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MKC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
